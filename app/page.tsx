    ...
    <div className="h-screen flex flex-col bg-gray-50 print:h-auto print:bg-white">
      ...
      <div className="hidden print:block w-full mb-6">
        <div className="border-2 border-gray-400 rounded-lg overflow-hidden bg-white">
          {/* 标题栏 */}
          <div className="bg-gradient-to-r from-blue-100 to-indigo-100 px-4 py-2 border-b-2 border-gray-400">
            <h3 className="text-sm font-bold text-gray-800">📋 需求清单</h3>
          </div>
          {/* 需求清单表格 */}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-100 border-b-2 border-gray-300">
                  <th className="text-left py-2 px-3 text-xs font-semibold text-gray-700 border-r border-gray-300 w-[35%]">需求名称</th>
                  <th className="text-center py-2 px-3 text-xs font-semibold text-gray-700 border-r border-gray-300 w-[10%]">类型</th>
                  <th className="text-center py-2 px-3 text-xs font-semibold text-gray-700 border-r border-gray-300 w-[8%]">复杂度</th>
                  <th className="text-center py-2 px-3 text-xs font-semibold text-gray-700 border-r border-gray-300 w-[8%]">优先级</th>
                  <th className="text-center py-2 px-3 text-xs font-semibold text-gray-700 border-r border-gray-300 w-[8%]">重点</th>
                  <th className="text-left py-2 px-3 text-xs font-semibold text-gray-700 w-[31%]">备注说明</th>
                </tr>
              </thead>
              <tbody>
                {(() => {
                  const renderRows: React.ReactElement[] = [];
                  const traverseNodes = (nodes: FunctionNode[], level: number = 0) => {
                    nodes.forEach((node, index) => {
                      ...
                      if (level === 0) {
                        nodeTypeLabel = '模块';
                        nodeTypeBg = 'bg-blue-100 text-blue-700';
                      } else if (hasChildren) {
                        nodeTypeLabel = '子模块';
                        nodeTypeBg = 'bg-cyan-100 text-cyan-700';
                      } else {
                        nodeTypeLabel = '功能菜单';
                        nodeTypeBg = 'bg-green-100 text-green-700';
                      }
                      
                      // 复杂度颜色
                      const complexityColor = node.complexity === '很高' ? 'text-red-600' :
                                              node.complexity === '高' ? 'text-orange-600' :
                                              node.complexity === '中' ? 'text-blue-600' : 'text-gray-600';
                      
                      // 优先级颜色
                      const priorityColor = node.priority === '很高' ? 'text-red-600' :
                                           node.priority === '高' ? 'text-orange-600' :
                                           node.priority === '中' ? 'text-blue-600' : 'text-gray-600';
                      
                      renderRows.push(
                        <tr key={node.id} className={`border-b border-gray-200 hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                          <td className="py-1.5 px-3 text-xs border-r border-gray-200" style={{ paddingLeft: `${level * 20 + 12}px` }}>
                          ...
                          <td className="py-1.5 px-3 text-center border-r border-gray-200">
                            <span className={`text-[10px] px-2 py-0.5 rounded font-medium ${nodeTypeBg}`}>
                              {nodeTypeLabel}
                            </span>
                          </td>
                          <td className={`py-1.5 px-3 text-center text-xs font-medium border-r border-gray-200 ${complexityColor}`}>
                            {node.complexity || '-'}
                          </td>
                          <td className={`py-1.5 px-3 text-center text-xs font-medium border-r border-gray-200 ${priorityColor}`}>
                            {node.priority || '-'}
                          </td>
                          <td className="py-1.5 px-3 text-center border-r border-gray-200">
                            {node.isImportant ? (
                      ...
                      // 如果有按钮，也要渲染
                      if (isLeaf && node.buttons && node.buttons.length > 0) {
                        node.buttons.forEach((button, btnIndex) => {
                          const btnComplexityColor = button.complexity === '很高' ? 'text-red-600' :
                                                      button.complexity === '高' ? 'text-orange-600' :
                                                      button.complexity === '中' ? 'text-blue-600' : 'text-gray-600';
                          
                          const btnPriorityColor = button.priority === '很高' ? 'text-red-600' :
                                                   button.priority === '高' ? 'text-orange-600' :
                                                   button.priority === '中' ? 'text-blue-600' : 'text-gray-600';
                          
                          renderRows.push(
                            <tr key={button.id} className="border-b border-gray-100 bg-blue-50/30">
                              <td className="py-1.5 px-3 text-xs border-r border-gray-200" style={{ paddingLeft: `${(level + 1) * 20 + 12}px` }}>
                                <div className="flex items-center gap-2">
                              ...
                              <td className="py-1.5 px-3 text-center border-r border-gray-200">
                                <span className="text-[10px] px-2 py-0.5 rounded font-medium bg-purple-100 text-purple-700">
                                  功能点
                                </span>
                              </td>
                              <td className={`py-1.5 px-3 text-center text-xs font-medium border-r border-gray-200 ${btnComplexityColor}`}>
                                {button.complexity || '-'}
                              </td>
                              <td className={`py-1.5 px-3 text-center text-xs font-medium border-r border-gray-200 ${btnPriorityColor}`}>
                                {button.priority || '-'}
                              </td>
                              <td className="py-1.5 px-3 text-center border-r border-gray-200">
                                {button.isImportant ? (
      ...
      <div className="flex-1 flex overflow-hidden print:flex-col print:overflow-visible print:gap-8">
        ...
        {/* 中间表格 */}
        <div className="flex-1 min-w-0 print:w-full print:mb-4 print:break-inside-avoid">
          <FunctionTable
            nodes={functionNodes}
            selectedNode={selectedNode}
            onNodesChange={saveToHistory}
